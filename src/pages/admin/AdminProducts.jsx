...
export default function AdminProducts() {
    ...
    // Track last auto-generated values to avoid overwriting user edits
    const lastAutoGenerated = useRef({ slug: '', seoTitle: '', seoDescription: '' });

    // Reset lastAutoGenerated when switching products (ID change)
    useEffect(() => {
        lastAutoGenerated.current = { slug: '', seoTitle: '', seoDescription: '' };
    }, [currentProduct.id]);

    // Automatically generate slug, seoTitle, and seoDescription when brand, name, category, or descriptions change
    useEffect(() => {
        const brand = (currentProduct.brand || '').trim();
        const name = (currentProduct.name || '').trim();
        
        if (!brand || !name) {
            setCurrentProduct(prev => {
                const updates = {};
                if (prev.slug === lastAutoGenerated.current.slug) {
                    updates.slug = '';
                }
                if (prev.seoTitle === lastAutoGenerated.current.seoTitle) {
                    updates.seoTitle = '';
                }
                if (prev.seoDescription === lastAutoGenerated.current.seoDescription) {
                    updates.seoDescription = '';
                }
                if (Object.keys(updates).length > 0) {
                    lastAutoGenerated.current = {
                        slug: updates.slug !== undefined ? '' : lastAutoGenerated.current.slug,
                        seoTitle: updates.seoTitle !== undefined ? '' : lastAutoGenerated.current.seoTitle,
                        seoDescription: updates.seoDescription !== undefined ? '' : lastAutoGenerated.current.seoDescription
                    };
                    return { ...prev, ...updates };
                }
                return prev;
        ...
        const generatedSlug = getTransliteratedSlug(brand, name);
        
        const generatedSeoTitle = isAccessory
        ...
        const generatedSeoDescription = isAccessory
            ? `Купить оригинальный аксессуар ${brand} - ${name} в интернет-магазине. ${cleanDesc}...`
            : `Купить оригинальный парфюм ${brand} - ${name} в интернет-магазине. ${cleanDesc}...`;

        setCurrentProduct(prev => {
            const updates = {};
            
            // Check if slug is empty OR matches the last auto-generated slug (was not modified by user)
            if (!prev.slug || prev.slug === lastAutoGenerated.current.slug) {
                updates.slug = generatedSlug;
            }
            
            // Same for seoTitle
            if (!prev.seoTitle || prev.seoTitle === lastAutoGenerated.current.seoTitle) {
                updates.seoTitle = generatedSeoTitle;
            }
            
            // Same for seoDescription
            if (!prev.seoDescription || prev.seoDescription === lastAutoGenerated.current.seoDescription) {
                updates.seoDescription = generatedSeoDescription;
            }
            
            if (Object.keys(updates).length > 0) {
                // Update the ref to track these new auto-generated values
                lastAutoGenerated.current = {
                    slug: updates.slug !== undefined ? updates.slug : lastAutoGenerated.current.slug,
                    seoTitle: updates.seoTitle !== undefined ? updates.seoTitle : lastAutoGenerated.current.seoTitle,
                    seoDescription: updates.seoDescription !== undefined ? updates.seoDescription : lastAutoGenerated.current.seoDescription
                };
                return { ...prev, ...updates };
            }
            return prev;
        });
    }, [currentProduct.brand, currentProduct.name, currentProduct.category, currentProduct.description, currentProduct.fullDescription]);
