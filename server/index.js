...
const downloadAutofillImage = async (url) => {
    ...
    if (!isDirectImage) {
        ...
        if (!pageRes.ok) {
            throw new Error(`Не удалось получить страницу аромата, статус: ${pageRes.status}`);
        }
...
app.post('/api/products/autofill', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        // Limit words function to guarantee maximum of 15 words for the short description
        ...
        res.json({
            description: limitWords(description || fullDescription, 15),
